...
if (isset($_POST['change_photo'])) {
    ...
    if (check_password_exist(($_POST['password']), $_SESSION['utilisateur_connecter_bibliothecaire']['id'])) {
        if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
            ...
            if ($_FILES["image"]["size"] <= 3000000) {
                ...
                if (in_array($file_ext, $allowed_ext)) {
                    ...
                    if (mise_a_jour_avatar($_SESSION['utilisateur_connecter_bibliothecaire']['id'], $profiledonnees["image"])) {

                    //       die(var_dump(recup_mettre_a_jour_informations_utilisateur($_SESSION['utilisateur_connecter_bibliothecaire']['id'])));
                        if (recup_mettre_a_jour_informations_utilisateur($_SESSION['utilisateur_connecter_bibliothecaire']['id'])) {
                            $_SESSION['utilisateur_connecter_bibliothecaire']=recup_mettre_a_jour_informations_utilisateur($_SESSION['utilisateur_connecter_bibliothecaire']['id']);
