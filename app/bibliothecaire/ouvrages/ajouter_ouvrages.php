...
<section class="section dashboard">
    <main id="main" class="main">
        <div class="row">
            ...
            if (!empty($_SESSION['ajout_ouvrage_errors'])) {
                $erreurs = $_SESSION['ajout_ouvrage_errors'];
            }
        ...
        <div class="row mt-5">
            <div class="col-md-12 bd-example">
                <form action="<?= PROJECT_ROM ?>bibliothecaire/ouvrages/traitement_ajouter_ouvrage" method="post" enctype="multipart/form-data">
                    ...
                    <div class="container">
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control <?= !empty($erreurs['titre_ouvrage']) ? 'is-invalid' : '' ?>" id="titre-ouvrage" value="<?= !empty($data['titre_ouvrage']) ? $data['titre_ouvrage'] : '' ?>" name="titre_ouvrage" placeholder="Veuillez entrer le titre de l'ouvrage">
                        ...
                        <div class="col-md-12">
                            ...
                            <span class="text-danger">*</span>
                            <input type="number" class="form-control <?= !empty($erreurs['nombre_exemplaire_ouvrage']) ? 'is-invalid' : '' ?>" id="nombre-exemplaire-ouvrage" value="<?= !empty($data['nombre_exemplaire_ouvrage']) ? $data['nombre_exemplaire_ouvrage'] : '' ?>" name="nombre_exemplaire_ouvrage">
                        ...
                        <div class="col-md-12">
                            ...
                            <span class="text-danger">*</span>
                            <select class="form-select select2bs4<?= isset($erreurs['auteur_principal_ouvrage']) ? 'is-invalid' : '' ?>" id="auteur-principal-ouvrage" name="auteur_principal_ouvrage">
                                ...
                                foreach ($liste_auteurs as $auteur) {
                                    $selected = $data['auteur_principal_ouvrage'] == $auteur['num_aut'] ? 'selected' : '';
                                    echo '<option value="' . $auteur['num_aut'] . '" ' . $selected . '>' . $auteur['nom_aut'] . ' ' . $auteur['prenoms_aut'] . '</option>';
                                }
                                ?>
                            </select>
                        ...
                        <div class="col-md-12">
                            ...
                            <span class="text-danger">*</span>
                            <input type="number" class="form-control <?= isset($erreurs['annee_publication']) ? 'is-invalid' : '' ?>" id="annee-publication" value="<?= !empty($data['annee_publication']) ? $data['annee_publication'] : '' ?>" name="annee_publication">
                        ...
                        <div class="col-md-12">
                            ...
                            <select class="form-select select2bs4 <?= isset($erreurs['auteurs_secondaires_ouvrage']) ? 'is-invalid' : '' ?>" id="auteurs-secondaires-ouvrage" name="auteurs_secondaires_ouvrage[]" multiple>
                                ...
                                foreach ($liste_auteurs as $auteur) {
                                ?>
                                    <option value="<?= $auteur['num_aut'] ?>" <?php echo !empty($data['auteurs_secondaires_ouvrage']) && in_array($auteur['num_aut'], $data['auteurs_secondaires_ouvrage']) ? 'selected' : '' ?>><?= $auteur['nom_aut'] . ' ' . $auteur['prenoms_aut'] ?></option>
                                <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select class="form-select select2bs4 <?= isset($erreurs['domaine_ouvrage']) ? 'is-invalid' : '' ?>" id="domaineListe" name="domaine_ouvrage[]" onchange="updateCategories()" multiple>
                                ...
                                foreach ($liste_domaine as $domaine_item) {
                                    ?>
                                    <option value="<?= $domaine_item['cod_dom'] ?>" <?php echo !empty($data['domaine_ouvrage']) && in_array($domaine_item['cod_dom'], $data['domaine_ouvrage']) ? 'selected' : '' ?> ><?= $domaine_item['lib_dom'] ?></option>
                                <?php
                                }
                                ?>
                            </select>
                        ...
                        <label for="more">Autre ouvrage à ajouter (Cochez cette case s'il y a un autre ouvrage à ajouter)</label>
...
unset($_SESSION['ajout_ouvrage_error'], $_SESSION['ajout_ouvrage_success'], $_SESSION['ajout_ouvrage_errors'], $_SESSION['data']);
?>