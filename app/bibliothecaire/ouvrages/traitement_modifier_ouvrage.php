...
$_SESSION['modifier_ouvrage_errors'] = [];
...
if (!empty($_POST['titre_ouvrage'])) {
    ...
    $_SESSION['modifier_ouvrage_errors']['titre_ouvrage'] = 'Champs requis.';
}
...
if (!empty($_POST['nombre_exemplaire_ouvrage'])) {
    ...
    $_SESSION['modifier_ouvrage_errors']['nombre_exemplaire_ouvrage'] = 'Champs requis.';
}
...
if (!empty($_POST['auteur_principal_ouvrage']) && $_POST['auteur_principal_ouvrage'] != 0) {
    ...
    $_SESSION['modifier_ouvrage_errors']['auteur_principal_ouvrage'] = 'Champs requis.';
}
...
if (!empty($_POST['annee_publication'])) {
    ...
    $_SESSION['modifier_ouvrage_errors']['annee_publication'] = 'Champs requis.';
}
    ...
    if (isset($_FILES["image_ouvrage"]) && $_FILES["image_ouvrage"]["error"] == 0) {
        ...
        if ($_FILES["image_ouvrage"]["size"] <= 3000000) {
            ...
            if (in_array(strtolower($file_ext), $allowed_ext)) {
                ...
                $_SESSION['modifier_ouvrage_errors']['image_ouvrage'] = "L'extension de votre image n'est pas pris en compte. <br> Extensions autorisées [ PNG/JPG/JPEG/GIF ]";
            }
        } else {

            $_SESSION['modifier_ouvrage_errors']['image_ouvrage'] = "Image trop lourde. Poids maximum autorisé : 3mo";
        }
    } else {
        $image_ouvrage = $_SESSION['ouvrage_image'];
    }
} else {
    $image_ouvrage = $_SESSION['ouvrage_image'];
}
...
if (empty($_POST['langue_ouvrage']) || $_POST['langue_ouvrage'] == 0) {
    ...
    $_SESSION['modifier_ouvrage_errors']['langue_ouvrage'] = 'Champs requis.';
...
if (is_array($_POST['domaine_ouvrage'])) {
    ...
    if (!empty($_POST['domaine_ouvrage']) && $_POST['domaine_ouvrage'][0] != 0) {
        ...
        $_SESSION['modifier_ouvrage_errors']['domaine_ouvrage'] = 'Champs requis.';
    }
} else {
    if (empty($_SESSION['global_bibliothecaire_error'])) {
        $_SESSION['global_bibliothecaire_error'] = 'Une action inattendue au niveau du champs Domaines bloque le processus.';
...
$errors = $_SESSION['modifier_ouvrage_errors'];

if (empty($errors) && empty($_SESSION['global_bibliothecaire_error'])) {

    maj_ouvrage($_SESSION['cod_ouv'], strtoupper($titre_ouvrage), intval(abs($nombre_exemplaire_ouvrage)), intval(abs($annee_publication)), $auteur_principal_ouvrage, $image_ouvrage);

    maj_assoc_ouvrage_lang_an_pub($_SESSION['cod_ouv'], $langue_ouvrage, $annee_publication);

    if (!empty($_POST['auteurs_secondaires_ouvrage'])) {
        foreach ($_POST['auteurs_secondaires_ouvrage'] as $key => $auteur_secondaire) {
            rem_old_assoc_ouvrage_aut_secondaires($_SESSION['cod_ouv']);
        }
    }

    if (!empty($_POST['auteurs_secondaires_ouvrage'])) {
        foreach ($_POST['auteurs_secondaires_ouvrage'] as $key => $auteur_secondaire) {
            maj_assoc_ouvrage_aut_secondaires($_SESSION['cod_ouv'], $auteur_secondaire);
        }
    }

    foreach ($_POST['domaine_ouvrage'] as $key => $dom) {
        rem_old_assoc_ouvrage_domaine($_SESSION['cod_ouv']);
    }

    foreach ($_POST['domaine_ouvrage'] as $key => $dom) {
        maj_assoc_ouvrage_domaine($_SESSION['cod_ouv'], $dom);
    }

    $_SESSION['global_bibliothecaire_success'] = 'Ouvrage modifié avec succès.';
