...
if (empty($errors)) {
    if (mettre_a_jour_informations_utilisateur(
        ...
        if (recup_mettre_a_jour_informations_utilisateur($data['id'])) {
            ...
            $_SESSION['global_membre_success'] = "Modifications effectuées avec succès";
        } else {
            $_SESSION['global_membre_error'] = "La modification a échoué. Veuillez vérifier votre mot de passe et réessayer.";
        }
    }
}

!empty($errors["mot_de_passe"]) ? $_SESSION['global_membre_error'] = $errors["mot_de_passe"] : '' ;
